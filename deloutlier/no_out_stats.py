...
def make_ez_stats(df, window_days, dow_window_days, suffix):
    ...
    grouped = df.groupby("air_store_num")["visitors"]
...
# load data
train = pd.read_csv('../output/outlier_cwrr_train.csv')
predict = pd.read_csv('../output/outlier_cwrr_predict.csv')
...
print("output to csv...")
train.to_csv('../output/outlier_w2_cwrrs_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/outlier_w2_cwrrs_predict.csv',float_format='%.6f', index=False)
