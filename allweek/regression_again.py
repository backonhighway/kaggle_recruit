...
# load data
train = pd.read_csv('../output/all_cw_train.csv')
predict = pd.read_csv('../output/all_cw_predict.csv')
...
print("output to csv...")
train.to_csv('../output/all_cwr_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/all_cwr_predict.csv',float_format='%.6f', index=False)

print("output to only regress...")
use_col = ["air_store_num", "visit_date", "quarter_regress", "year_regress"]
train = train[use_col]
predict = predict[use_col]
train.to_csv('../output/regress_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/regress_predict.csv',float_format='%.6f', index=False)