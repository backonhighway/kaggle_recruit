...
def make_ez_stats(df, window_days, dow_window_days, suffix):
    ...
    df = get_ez_stats(df, grouped, window_days, col_names, SHIFT_WEEK * 7)

    col_names = ["dow_" + s for s in col_names]
    grouped = df.groupby(["air_store_num", "dow"])["visitors_nan"]
    df = get_ez_stats(df, grouped, dow_window_days, col_names, SHIFT_WEEK)
    return df
...
def get_filled_rolling(df: pd.DataFrame, col_name_list):
    for col_name in col_name_list:
        ...
        grouped = df.groupby(["air_store_num", "dows"])[col_name]
...
def get_ewm(df):
    grouped = df.groupby(["air_store_num", "dows"])["visitors_nan"]
    df["ewm"] = grouped.transform(lambda g: calc_ewm(g, 0.1).shift(SHIFT_WEEK))
    grouped = df.groupby(["air_store_num", "dows"])["log_visitors_nan"]
    df["log_ewm"] = grouped.transform(lambda g: calc_ewm(g, 0.1).shift(SHIFT_WEEK))
    return df
...
joined["visitors_nan"] = np.where(joined["visit_date"] <= "2017-03-04", joined["visitors"], np.NaN)
joined["log_visitors_nan"] = np.where(joined["visit_date"] <= "2017-03-04", joined["log_visitors"], np.NaN)
...
print("output to csv...")
train.to_csv('../output/all_cwrrm_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/all_cwrrm_predict.csv',float_format='%.6f', index=False)
