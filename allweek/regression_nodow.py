...
# load data
train = pd.read_csv('../output/all_cwr_train.csv')
predict = pd.read_csv('../output/all_cwr_predict.csv')
...
dummy = pd.get_dummies(joined["dows"])
dummy.columns = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"]
joined = joined.join(dummy)
...
print("output to csv...")
train.to_csv('../output/all_cwrr_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/all_cwrr_predict.csv',float_format='%.6f', index=False)

print("output to only regress...")
use_col = ["air_store_num", "visit_date", "quarter_regress_no_dow", "year_regress_no_dow"]
train = train[use_col]
predict = predict[use_col]
train.to_csv('../output/regress_no_dow_train.csv',float_format='%.6f', index=False)
predict.to_csv('../output/regress_no_dow_predict.csv',float_format='%.6f', index=False)