...
def get_submission(file_prefix, reserve_col):
    ...
    model = models[0]
    good_stores = train[train["first_appear"] < "2017-04-01"]
    unique_good_store = good_stores["air_store_num"].unique()
    print(unique_good_store)
    exit(0)
    ...
    submission = pd.DataFrame({
            "id": predict['id'],
            "visitors": y_pred,
            "visitors_good": y_pred_good,
        })

    submission["air_store_num"] = submission['id'].map(lambda x: str(x).split('_')[2])
